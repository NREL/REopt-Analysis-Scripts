...
#change the optimality tolerance
opt_tol = [0.99, 0.99, 0.99, 0.99]
site_analysis = []
...
for i in sites_iter
    for tech in ["Site"]
        if haskey(site_analysis[i][2], tech)
            println("Site  Emissions Reduction = ", site_analysis[i][2][tech]["lifecycle_emissions_reduction_CO2_fraction"])
        end
    end
    #for tech in ["ElectricHeater"]
        #if haskey(site_analysis[i][2], tech)
            #println("Site size (MMBtu/hr) = ", site_analysis[i][2][tech]["size_mmbtu_per_hour"])
            #println("Site  Thermal Production (MMBtu) = ", site_analysis[i][2][tech]["annual_thermal_production_mmbtu"])
        #end
    #end
    for tech in ["ExistingBoiler"]
        if haskey(site_analysis[i][2], tech)
            println("Site  Annual Fuel Consumption (MMBtu) = ", site_analysis[i][2][tech]["annual_fuel_consumption_mmbtu"])
        end
    end
end
"""
